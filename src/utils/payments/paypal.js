...
export async function paypalPayment(value, id, currency) {
  try {
    ...
    return `https://${
      data.ex1 === 'sandbox' ? 'sandbox' : 'www'
    }.paypal.com/checkoutnow/?token=${payment.id}`;
  } catch (err) {
    return err;
  }
}

export async function executePaymentPaypal(payerId, paymentId) {
  const data = await Gateway.findOne({ where: { value: 'paypal' } });
  paypal.configure({
    mode: data.ex1,
    client_id: data.apiKey,
